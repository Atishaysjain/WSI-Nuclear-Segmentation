...
class InferManager(base.InferManager):
    ...
    ####
    def process_file_list(self, run_args, save_mat):
        """
        ...
        file_path = self.input_file # line added 
        assert os.path.exists(file_path), 'Not Detected Any Files From Path' # line added 
        ...
        def proc_callback(results, save_mat):
        ...
        if available_ram < 0:
            # break
            assert False, "available ram is less than 0"
        ...
        for file_idx, file_path in enumerate(use_path_list):
            ...
            if proc_pool is not None:
                ...
                proc_output = _post_process_patches(*func_args)
                proc_callback(proc_output, save_mat)
        ...
        if proc_pool is not None:
            ...
            for future in as_completed(future_list):
                ...
                if future.exception() is not None:
                    ...
                    file_path = proc_callback(future.result(), save_mat)
                    log_info("Done Assembling %s" % file_path)
